...
async def check_online_payment(callback: types.CallbackQuery):
    ...
    if payment_kind == "renew":
        ...
        await answer_rich(callback.message, t("renew_done", service_name=service_name, **_renewal_confirmation_values(panel_data, added_volume, added_days)))
        try:
            await alerts.log_renewal_to_channel(callback.bot, db.get_user_by_id(cfg["user_id"]) or {}, cfg, panel_data, int(payment.get("price") or 0))
        except Exception:
...
async def renew_pay_wallet(callback: types.CallbackQuery, state: FSMContext):
    ...
    await answer_rich(callback.message, t("renew_done", service_name=service_name, **_renewal_confirmation_values(panel_data, added_volume, added_days)))
    try:
        await alerts.log_renewal_to_channel(callback.bot, user, cfg, panel_data, price)
    except Exception:
