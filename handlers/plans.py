...
def _renewal_confirmation_values(panel_data: dict | None, added_volume: float, added_days: int) -> dict:
    """مقدار قبلی/افزوده/نهایی را از snapshot واقعی پنل برای پیام تأیید محاسبه می‌کند."""
    import math, time
    data=panel_data or {}
    ...
    if total is not None:
        try:
            ...
            if new_gb <= 0:
                ...
                previous_volume=new_volume=f"{new_gb:g} گیگ"
        except Exception:
            previous_volume=new_volume="نامشخص"
    else:
        previous_volume=new_volume="نامشخص" if added_volume else "بدون تغییر"
    if not added_volume and total is None:
        previous_volume=new_volume="بدون تغییر"
    expire=data.get("expire")
    if expire:
        try:
            new_days=max(0,int(math.ceil((int(expire)-time.time())/86400)))
            if added_days:
                previous_days=f"{max(0,new_days-int(added_days))} روز"; new_days_text=f"{new_days} روز"
            else:
    ...
    return {
        ...
        "previous_volume": previous_volume, "new_volume": new_volume,
        "added_days": f"{int(added_days)} روز" if added_days else "بدون تغییر",
        "previous_days": previous_days, "new_days": new_days_text,
...
async def check_online_payment(callback: types.CallbackQuery):
    ...
    if payment_kind == "renew":
        ...
        await answer_rich(callback.message, t("renew_done", service_name=payment.get("plan_name") or "سرویس", **_renewal_confirmation_values(panel_data, float(payload.get("volume_gb") or 0), int(payload.get("days") or 0))))
        return
...
async def receive_purchase_receipt(message: types.Message, state: FSMContext):
    ...
    if data.get("renew_invoice_id"):
        ...
        renew_details=(f"دسته: {renew_cat.get('name')}\n" if renew_cat else "") + f"حجم تمدید: +{float(payload.get('volume_gb') or 0):g} گیگ\nروز تمدید: +{int(payload.get('days') or 0)} روز"
        admin_renew_text=(f"💳 رسید تمدید سرویس\n\n👤 {message.from_user.full_name}\n🆔 {message.from_user.id}\n🔁 {data.get('renew_service_name','سرویس')}\n📦 {renew_details}\n💰 {invoice['price']:,} تومان")
...
async def renew_pay_wallet(callback: types.CallbackQuery, state: FSMContext):
    ...
    _save_renewal_snapshot(cfg, panel_data)
    await state.clear()
    await answer_rich(callback.message, t("renew_done", service_name=data.get("renew_service_name") or cfg.get("plan"), **_renewal_confirmation_values(panel_data, float(data.get("renew_volume_gb") or 0), int(data.get("renew_days") or 0))))
...
async def receive_crypto_receipt(message:types.Message,state:FSMContext):
    ...
    if is_crypto_renewal:
        ...
        package_name = (renew_cfg or {}).get("plan") or (category or {}).get("name") or "سرویس"
        if category:
            package_name += f"\nدسته: {category.get('name')}"
        package_name += f"\nتمدید: +{float((data.get('crypto_payload') or {}).get('volume_gb') or 0):g} گیگ | +{int((data.get('crypto_payload') or {}).get('days') or 0)} روز"
    else:
