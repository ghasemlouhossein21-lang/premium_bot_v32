...
async def approve_renewal(callback: types.CallbackQuery):
    ...
    try: await send_rich(callback.bot,int(receipt["telegram_id"]),user_text("renew_done",service_name=service_username,**_renewal_confirmation_values(panel_data,volume,days)))
    except Exception: pass
    try:
        renew_user=db.get_user(receipt["telegram_id"]) or {}
        await alerts.log_renewal_to_channel(callback.bot, renew_user, cfg or {}, panel_data, int(receipt.get("amount") or 0))
    except Exception:
